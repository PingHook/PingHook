...
async def send_telegram(chat_id: str, label: str, payload: str) -> bool:
    text = format_telegram_message(label, payload)
    await bot.send_message(chat_id=int(chat_id), text=text)
    return True


async def send_slack(webhook_url: str, label: str, payload: str) -> bool:
...
async def send_discord(webhook_url: str, label: str, payload: str) -> bool:
...
async def dispatch(channel: dict, label: str, payload: str) -> bool:
    try:
        ch_type = channel["type"]
        dest    = channel["destination"]
        if ch_type == "telegram":
            return await send_telegram(dest, label, payload)
        elif ch_type == "slack":
            return await send_slack(dest, label, payload)
        elif ch_type == "discord":
            return await send_discord(dest, label, payload)
        return False
...
async def validate_and_save_webhook(
    ...
    if channel_type == "slack":
        success = await send_slack(webhook_url, "pinghook-test", test_payload)
    elif channel_type == "discord":
        success = await send_discord(webhook_url, "pinghook-test", test_payload)
    else:
